...
`keymap-guide.html` が、電源の入れ方・レイヤーごとのキー配置・動作確認の手順を
1ページにまとめたもの。ブラウザで開くだけで読める（GitHub 上ではソースが表示される
だけなので、ダウンロードして開いてもらう）。
...
**副作用**: ボールを動かした直後 500ms 以内に `S` `D` `F` `J` `K` `L` を打つと、文字では
なくクリックになる。気になる場合は `boards/shields/mona2/mona2.dtsi` の
`<&zip_temp_layer 5 500>` の数値を小さくするか、行をコメントアウトに戻す。
